package bsu.rfe.java.group8.lab4.Romanova.varA16;
...
и Y - сколько пикселов
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point: graphicsData) {
            ...
            int intervalCount = 0;
            for (Double[] point2: graphicsData) {
                double y = point2[0] * Math.sin(point2[0]);
                intervalSum += point2[1];
                intervalCount++;
            }
            double intervalAverage = intervalSum / intervalCount;
        	
        	  double num = Math.abs(point[0].doubleValue());

              if (point[1] > 2 * intervalAverage) {
                  // Значение функции в точке превышает в более чем 2 раза среднее значение функции на всем интервале
                  // Изменение цвета маркера на зеленый
...
//              else  canvas.setColor(Color.RED);
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if (minX<=0.0 && maxX>=0.0) {
          ...
          canvas.drawString(originLabel, (float) (originLabelPos.getX() - originBounds.getWidth() - 10),
